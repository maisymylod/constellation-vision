...
- **The model is trained from scratch.** No pretrained weights, no download.
  `make train` fits the model on CPU and exports `segmenter.onnx`, which is the
  committed model; the torch `.pt` checkpoint is regenerated by `make train` and
  not committed.
